...
class DesiredTrajectoryNode:
    def __init__(self):
        ...
        self.lambda1, self.lambda2, self.lambda3 = 10.0, 10.0, 7.5
        ...
        self.s_int1, self.s_int2, self.s_int3 = 0.0, 0.0, 0.0
    ...
    def generate_torque(self):
        ...
        if current_time <= tf:
            self.s_int1 += self.k21 * np.tanh(s1) * self.dt
            self.s_int2 += self.k22 * np.tanh(s2) * self.dt
            self.s_int3 += self.k23 * np.tanh(s3) * self.dt

        u1 = -self.k11 * np.sqrt(np.abs(s1)) * np.tanh(s1) - self.s_int1
        u2 = -self.k12 * np.sqrt(np.abs(s2)) * np.tanh(s2) - self.s_int2
        u3 = -self.k13 * np.sqrt(np.abs(s3)) * np.tanh(s3) - self.s_int3
